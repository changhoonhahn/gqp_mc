...
\edits{
    Another straightforward probabilistic analysis would be to use the
    posteriors on $M_*$ to measure the \emph{probabilistic} SMF of BGS. 
    With full posteriors, we can accurately probe even the lowest
    signal-to-noise regime so the SMF will be accurate even at the lowest mass
    end, down to ${\sim}10^{7} M_\odot$ (Figure~\ref{fig:bgs_mstar}). 
    Precise constraint on the SMF of dwarf galaxies has important implications
    for both galaxy evolution and cosmology. 
    Probabilistic analyses can extend to higher dimensions. 
    Posteriors on $M_*$ and SFR can be used to infer the 
}
