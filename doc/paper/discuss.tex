...
\edits{
    ...
    $p(M_* \given {\bfi X}_i)$, to measure the \emph{probabilistic} SMF of BGS.
    With full posteriors, we can probe even the lowest signal-to-noise regime
    so the SMF will be accurate at the lowest mass end, down to 
    ${\sim}10^{7} M_\odot$ (Figure~\ref{fig:bgs_mstar}). 
    This will be particularly impactful since a precise constraint on the SMF
    of dwarf galaxies has important implications for both galaxy evolution and
    cosmology. 
}
...
\edits{
    ...
    can be used to measure the probabilistic SFS. 
    ...
    galaxy properties given observations, $p(\theta | \{X_i\})$, which contains
    the full statistical power of observations. 
