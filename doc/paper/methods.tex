...
In calculating $\mathcal{L}^{\rm spec}$, we exclude 40\AA wavelength range
surrounding the OII, H$\beta$, OIII, and H$\alpha$ emission lines since our SED
model does not model gas emissions.
...
observations.
The entire PROVABGS SED modeling pipeline, which includes the neural emulators
and parameter inference framework, is publicly available at
