...
\edits{
    so 43 SSPs contribute to the total galaxy SED.
} 
...
\edits{
    Since we model galaxies as a linear combination of SSPs, we do not model
    gas emission.  
...
\edits{
    ...
    We represent the model luminosity using the first $N_{\rm basis}$
    eigenspectra and their corresponding PCA coefficients. 
    We use $N_{\rm basis}$ = 50, 50, 50, and 30 for the wavelength bins.
    A NN is then trained on the set of $N_{\rm model}$ models to
    derive a mapping from the $N_{\rm SPS}=12$ SPS parameters to the 
    $N_{\rm basis}$ PCA coefficients for each wavelength bin. 
}
