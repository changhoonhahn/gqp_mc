...
\caption{
    ...
    For each simulated galaxy, we plot 10 samples drawn from the marginalized
    posterior of $\theta$.
...
We also include violin plots of $\hat{\theta}$ for a handful of randomly
selected galaxies.
The width of the violin plot represents the marginalized posterior distribution
of $\theta$.
...
Third, we fix velocity disperions to 0 km/s in our SPS model. 
Lastly, we use different dust prescriptions: \cite{mathis1983} dust
attenuation curve in the forward model and the \cite{kriek2013} curve in the
SPS model. 
...
($\sigma_{\Delta_{\theta}}$) of the distribution that represent the accuracy
and precision of the inferred posteriors for the galaxy population. 
We can infer the population hyperparameters, $\mu_{\Delta_{\theta}}$ and
$\sigma_{\Delta_{\theta}}$, using a hierarchical Bayesian
...
\begin{figure}
\begin{center}
    \includegraphics[width=0.9\textwidth]{figs/etas_v2.pdf}
    \caption{
        ...
        (green). 
        We derive $\eta_\Delta$ from the posteriors using a Hierarchical
        Bayesian approach. 
...
\begin{figure}
\begin{center}
    \includegraphics[width=\textwidth]{figs/etas_photo.pdf}
    \caption{
...
\begin{center}
    ...
    \caption{
        ...
        and $\avgsfr$. 
        We present $\mu_{\Delta_{\theta}}$ (top) and $\sigma_{\Delta_{\theta}}$
        (bottom) in ($M_*$, $\avgsfr$) bins for $\log M_*$, $\log\avgsfr$,
        $\log Z_{\rm MW}$, $t_{\rm age, MW}$ and $\tau_{\rm ISM}$ from left to
        right. 
...
underlying dependences in the inferred galaxy properties on the  
$M_*$-${\rm SFR}$ plane. 
...
We only show bins with more than 10 galaxies. 
...
We also find no significant dependence on SNR --- neither $r_{\rm
...
significantly broader, $\sigma_{\Delta_{\log M_*}}{\sim}0.25 - 0.3$ dex.
...
$\log\avgsfr$ by $\mu_{\Delta_{\log\avgsfr}}{>}0.5$ dex for the reddest
galaxies with $g-r > 1.5$ and $r-z> 0.6$.
The constraints for these galaxies are also significantly less precise:
$\sigma_{\Delta_{\log\avgsfr}} \sim 0.5$ dex. 
The bias is also apparent in Figure~\ref{fig:etas_msfr}, where we significantly
overestimate $\avgsfr$ for quiescent galaxies. 
...
Including spectra reduces the overall bias on $\zmw$ by $\sim$0.3 dex. 
...
contribution of the likelihood from the spectra is lower and the prior on
...
The $\zmw$ posteriors further underscore the constraining power of DESI
spectra. \\
...
correlation between $r-z$ and true $\tage$: simulated galaxies with 
$r - z > 0.6$  have overall older stellar populations. 
...
significantly on the true $\tauism$ value. 
