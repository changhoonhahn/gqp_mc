...
class iFSPS(Fitter): 
	...
	def _emcee(self, lnpost_fn, lnpost_args, lnpost_kwargs, nwalkers=100,
		...
		''' Runs MCMC (using emcee) for a given log posterior function and monitors
			the convergence real-time.
		'''
		...
		#if not silent: print(f'Adaptive convergence step: {ADAPTIVE_STEP}')
		...
		min_result = op.minimize(
				_lnpost,
				0.5 * (lnpost_kwargs['prior'].max + lnpost_kwargs['prior'].min),
				args=lnpost_args,
				method='Nelder-Mead',
		...
		#convergence flag
		convergent = False
		if niter < MINIMUM_IT: niter = MINIMUM_IT #If niter is less than the minimum iteration, which shoul be about ~40000, set it equal to MINIMUM_IT
		...
		for idx in range(niter//STEP):
			...
			if (nwalkers * STEP * (idx+1)) > MINIMUM_IT:   #ACM is exectued only after the mcmc chain has been iterated more than MINMIUM_IT number.
		...
		if not convergent:								
			if (STEP * (idx + 1) < niter):
				...
				if not silent:
					if convergent: print(f'Converged; PSRF: {PSRF}, Iteration: {niter}, Convergence stability couldn\'t be checked')
