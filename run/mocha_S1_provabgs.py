...
second simplest test on PROVABGS mocks generated using jupyter notebook:
https://github.com/changhoonhahn/provabgs/blob/main/nb/provabgs_mocks.ipynb

The mocks are generated using provabgs. All at redshift z=0.2. They have a
...
def run_mcmc(i_obs): 
    if corrp: 
        fchain_npy  = os.path.join(dat_dir, 'provabgs_mock.noisy.%i.cp.chain.npy' % i_obs)
        fchain_p    = os.path.join(dat_dir, 'provabgs_mock.noisy.%i.cp.chain.p' % i_obs)
    else: 
        fchain_npy  = os.path.join(dat_dir, 'provabgs_mock.noisy.%i.chain.npy' % i_obs)
        fchain_p    = os.path.join(dat_dir, 'provabgs_mock.noisy.%i.chain.p' % i_obs)
