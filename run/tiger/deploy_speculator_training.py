...
def deploy_trainspeculator_job(model, i_wave, n_pcas, Ntrain, Nlayer, Nunits):
    ...
    '''
    cntnt = '\n'.join(["#!/bin/bash", 
        "#SBATCH -J train_spec_%s_%i_%i_%i_%ix%i" % (model, i_wave, n_pcas, Ntrain, Nlayer, Nunits),  
        "#SBATCH --exclusive",
        "#SBATCH --nodes=1",
        ...
        "#SBATCH --export=ALL",
        "#SBATCH --output=ofiles/train_spec_%s_%i_%i_%i_%ix%i.o" % (model, i_wave, n_pcas, Ntrain, Nlayer, Nunits),  
        "#SBATCH --mail-type=all",
